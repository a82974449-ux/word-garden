...
async function apiPost(path, body) {
  ...
  if (!res.ok) {
    ...
    try {
      ...
      detail = j && (j.detail || j.error) ? String(j.detail || j.error) : "";
    } catch (e2) {
...
async function classifyWord(word, arabic) {
  try {
    ...
    return {
      ...
      emoji: DEFAULT_EMOJI,
      ok: false,
      rateLimited: e && e.status === 429,
      debugDetail: e && (e.detail || e.message),
    };
  }
}
...
export default function App() {
  ...
  async function addWord() {
    ...
    const { level, arabic, emoji, ok, rateLimited, debugDetail } = await classifyWord(en, arInput);
    if (!arabic) {
      const base = rateLimited ? "تم الوصول للحد الأقصى من الطلبات مؤقتًا، حاول بعد قليل." : ok ? "لم يصل رد كافٍ، الرجاء إدخال الترجمة يدويًا." : "تعذّر الاتصال بخدمة التصنيف، الرجاء إدخال الترجمة يدويًا.";
      setAddError(base + (debugDetail ? `\n[تفاصيل تقنية: ${debugDetail}]` : ""));
      ...
      <div className="px-5 mt-5">
        {!showAdd ? (
          ...
          <div style={{ background: PALETTE.card, border: `1px solid ${PALETTE.line}`, borderRadius: "14px" }} className="p-4 flex flex-col gap-2">
            ...
            {addError && <p style={{ color: PALETTE.wilt, whiteSpace: "pre-wrap" }} className="text-xs">{addError}</p>}
